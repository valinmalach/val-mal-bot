...
migrated with [Alembic](https://alembic.sqlalchemy.org/). This is step one of
consolidating the parquet files and the scattered hardcoded values into one
place: **the schema exists, nothing reads from it yet.** The bot still runs
entirely off `data/*.parquet` and `constants.py`.
...
Nothing connects at import time, so a missing `DATABASE_URL` cannot break bot
startup while the migration is still in progress.
...
| `app_setting` | `GUILD_ID`, `OWNER_ID`, `BROADCASTER_USERNAME`, the `$` command prefix, the backup directory in `cogs/tasks.py`, the Twitch scope list in `token_manager.py`, and the two Twitch account IDs (see below) |
...
They are already gone from `.env.example`, since nothing deploys until this
migration is finished. They stay in the working `.env` only because the code
still reads them with `os.getenv`; that goes away with step 5 below.
...
That only becomes true once the read path exists: a cached lookup with
invalidation, so an edit takes effect without a restart. Until then the values
are still imported from `constants.py` at module load.
...
4. Token cutover: point `TwitchTokenManager` at `oauth_token`, re-authorize the
   three identities, then delete `data/twitch/*.txt`.
5. Read configuration from `app_setting` / `discord_channel` / `discord_role`
   through a cache with invalidation, instead of importing `constants.py`.
6. Admin commands to edit channel, role and Twitch IDs from Discord, so changing
   one never needs database access.
