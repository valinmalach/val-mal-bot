"""Seed the configuration tables.

Inserts the 158 rows seed.py used to write on every boot. Conflict-tolerant, so
it does nothing on a database that has already been seeded, and every value is a
literal: a revision cannot read seed_data or a model that will keep changing.
...
# Column stubs, not the models: what a table looked like when this revision was
# written is what it has to keep inserting into.
_discord_channel = sa.table(
    "discord_channel",
    sa.column("key", sa.String),
    sa.column("channel_id", sa.BigInteger),
    sa.column("description", sa.String),
)
_discord_embed = sa.table(
    "discord_embed",
    sa.column("key", sa.String),
...
_discord_role = sa.table(
    "discord_role",
    sa.column("key", sa.String),
    sa.column("role_id", sa.BigInteger),
    sa.column("name", sa.String),
    sa.column("emoji", sa.String),
    sa.column("custom_id", sa.String),
    sa.column("embed_key", sa.String),
    sa.column("position", sa.Integer),
    sa.column("assignable", sa.Boolean),
    sa.column("description", sa.String),
)
_discord_embed_field = sa.table(
    "discord_embed_field",
    sa.column("embed_key", sa.String),
    sa.column("position", sa.Integer),
    sa.column("name", sa.String),
    sa.column("value", sa.Text),
    sa.column("inline", sa.Boolean),
)
...
_discord_auto_response = sa.table(
...
_DISCORD_CHANNEL_ROWS = [
    {"key": "audit_logs", "channel_id": 1291775826655707166, "description": None},
    {"key": "bot_admin", "channel_id": 1346408909442781237, "description": None},
    {"key": "dm_requests", "channel_id": 1292413187270115328, "description": None},
    {"key": "food", "channel_id": 1291026325045248101, "description": None},
    {"key": "pets", "channel_id": 1291027524947546164, "description": None},
    {"key": "promo", "channel_id": 1378917167336001606, "description": None},
    {"key": "ranting", "channel_id": 1291026750947590266, "description": None},
    {"key": "roles", "channel_id": 1285277373167570946, "description": None},
    {"key": "rules", "channel_id": 1285275553611517963, "description": None},
    {"key": "shoutouts", "channel_id": 1291026077287710751, "description": None},
    {"key": "stream_alerts", "channel_id": 1285276760044474461, "description": None},
    {"key": "welcome", "channel_id": 1285276874645438544, "description": None},
]
_DISCORD_EMBED_ROWS = [
    {
        "key": "rules",
...
_DISCORD_EMBED_FIELD_ROWS = [
    ...
    },
    {
        ...
        "position": 2,
        "name": "",
        "value": "* No doxxing or sharing of personal information (birthdays and other significant dates are exceptions). Barring {channel:food} and {channel:pets} and possibly scenery, no sharing of irl pictures, videos or anything. This might be a bit overkill, but I have known people to have shared an innocent irl picture without realising there's doxxable info in the background. I just want to ensure a completely dox-free environment.",
        "inline": False,
    },
    {
        "embed_key": "rules",
    ...
    },
    {
        ...
        "position": 8,
        "name": "",
        "value": "* Avoid discussions about severe mental health issues as it may be triggering for others. If you are in need of help, please seek professional help. I am not a therapist, and this isn't the place for that. Exceptions apply to {channel:ranting} **to a certain extent**. Everything has its limits. The channel can and will be removed if it's abused.",
        "inline": False,
    },
    {
        "embed_key": "rules",
        "position": 9,
        "name": "",
        "value": "* Speaking of {channel:ranting}, spoiler when necessary. Especially any topics that might even be remotely triggering. If you are unsure, just spoiler it. There's no harm in spoilering.",
        "inline": False,
    },
    {
        "embed_key": "rules",
    ...
    },
    {
        ...
        "position": 17,
        "name": "",
        "value": "**Click the button below to accept the rules to get the {role:follower} role and gain access to the server. By clicking the button, I expect you to have read and understood the rules.**",
        "inline": False,
    },
    {
        "embed_key": "ping_roles",
...
_TWITCH_COMMAND_RESPONSE_ROWS = [
    {
        "command_name": "lurk",
        "position": 0,
        "message": "{chatter} has gone to lurk. Eat, drink, sleep, water your pets, feed your plants. Make sure to take care of yourself and stay safe while you're away!",
    },
    {
        "command_name": "unlurk",
        "position": 0,
        "message": "{chatter} has returned from their lurk. Welcome back! Hope you had a good break and are ready to hang out again!",
    },
    {
...
_TWITCH_COMMAND_COMPONENT_ROWS = [
...
_APP_SETTING_ROWS = [
    ...
    },
    {
        ...
        "value": '["channel:bot", "channel:read:ads", "channel:read:redemptions", "moderator:manage:announcements", "moderator:manage:banned_users", "moderator:manage:blocked_terms", "moderator:manage:chat_messages", "moderator:manage:chat_settings", "moderator:manage:shoutouts", "moderator:manage:unban_requests", "moderator:manage:warnings", "moderator:read:chatters", "moderator:read:followers", "moderator:read:moderators", "moderator:read:vips", "user:bot", "user:read:chat", "user:write:chat"]',
        "value_type": "json",
        "description": "Scopes requested for the app access token",
    },
]
_MESSAGE_TEMPLATE_ROWS = [
    {
    ...
    {
        "key": "twitch_shoutout",
        "content": "Go follow {name} at https://www.twitch.tv/{login}. They were last seen playing {game}.",
    ...
    },
    {
        ...
        "content": "Thank you for following, {user}! valinmKiss Your support means a lot to me! I hope you enjoy your stay! valinmKiss",
        "description": "channel.follow event",
    },
    {
        "key": "twitch_ad_break_warning",
        "content": "The next ad break will start in 5 minutes! Feel free to take a quick break while the ads run! valinmHydrate",
        "description": "Sent five minutes before a scheduled ad break",
    },
    {
        "key": "twitch_raid_out",
        "content": "We just raided {name}. In case you got left behind, you can find them here: {url} valinmRaid",
    ...
    {"key": "admin_nuking", "content": "Nuking channel...", "description": "/nuke"},
    {
    ...
    },
    {
        ...
        "content": "A {minutes} minute ad break is starting! Thank you for sticking with us through this break! valinmArrive Ads help support my content. Consider subscribing to remove ads and support the stream!",
    ...
    },
    {
        ...
        "content": "Have a great rest of your day! valinmKiss Don't forget to stay hydrated and take care of yourself! valinmHydrate",
...
def upgrade() -> None:
    op.execute(
        insert(_discord_channel).values(_DISCORD_CHANNEL_ROWS).on_conflict_do_nothing()
    )
    op.execute(
        insert(_discord_embed).values(_DISCORD_EMBED_ROWS).on_conflict_do_nothing()
    )
    op.execute(
        insert(_discord_role).values(_DISCORD_ROLE_ROWS).on_conflict_do_nothing()
    )
    op.execute(
        insert(_discord_embed_field)
        .values(_DISCORD_EMBED_FIELD_ROWS)
        .on_conflict_do_nothing()
    )
    op.execute(
        insert(_twitch_command).values(_TWITCH_COMMAND_ROWS).on_conflict_do_nothing()
    )
    op.execute(
        insert(_twitch_command_response)
        .values(_TWITCH_COMMAND_RESPONSE_ROWS)
        .on_conflict_do_nothing()
    )
    op.execute(
        insert(_twitch_command_component)
        .values(_TWITCH_COMMAND_COMPONENT_ROWS)
        .on_conflict_do_nothing()
    )
    op.execute(insert(_app_setting).values(_APP_SETTING_ROWS).on_conflict_do_nothing())
    op.execute(
        insert(_message_template)
        .values(_MESSAGE_TEMPLATE_ROWS)
        .on_conflict_do_nothing()
    )
    op.execute(
        insert(_discord_auto_response)
        .values(_DISCORD_AUTO_RESPONSE_ROWS)
        .on_conflict_do_nothing()
    )


def downgrade() -> None:
    # Parent first: embed fields and the twitch_command child rows are
    # ON DELETE CASCADE, so they go with the row that owns them.
    op.execute(
        sa.delete(_discord_auto_response).where(
            _discord_auto_response.c.trigger.in_(
                [row["trigger"] for row in _DISCORD_AUTO_RESPONSE_ROWS]
            )
        )
    )
    op.execute(
        sa.delete(_message_template).where(
            _message_template.c.key.in_([row["key"] for row in _MESSAGE_TEMPLATE_ROWS])
        )
    )
    op.execute(
        sa.delete(_app_setting).where(
            _app_setting.c.key.in_([row["key"] for row in _APP_SETTING_ROWS])
        )
    )
    op.execute(
        sa.delete(_twitch_command).where(
            _twitch_command.c.name.in_([row["name"] for row in _TWITCH_COMMAND_ROWS])
        )
    )
    op.execute(
        sa.delete(_discord_role).where(
            _discord_role.c.key.in_([row["key"] for row in _DISCORD_ROLE_ROWS])
        )
    )
    op.execute(
        sa.delete(_discord_embed).where(
            _discord_embed.c.key.in_([row["key"] for row in _DISCORD_EMBED_ROWS])
        )
    )
    op.execute(
        sa.delete(_discord_channel).where(
            _discord_channel.c.key.in_([row["key"] for row in _DISCORD_CHANNEL_ROWS])
        )
    )
