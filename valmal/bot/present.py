...
def quoted(value: str) -> str:
    ...
    Nearly every caller echoes back something *rejected* (a login that failed
    its grammar, say), which is the value most likely to carry markup.

    A code span rather than escape_markdown, and the two do not compose. A span
    renders nothing inside it, covering markdown *and* a mention, which
    escape_markdown does not touch (`<@id>` would ping). A span cannot survive a
    backtick, and escaping one does not help: a backslash is literal inside a
    span, so an escaped backtick still closes it. So backticks are removed and
    the span does the rest.
    """
