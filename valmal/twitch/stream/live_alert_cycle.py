...
from valmal.twitch.stream.live_alert_close import fetch_vod, gather_close_info
...
async def _close(
    ...
    own_stream, channel_info, url = await gather_close_info(
        broadcaster_id, stream_id, stream, user_info
    )

    vod = await fetch_vod(broadcaster_id, stream_id)
    embed = offline_embed(
        own_stream,
        vod,
        channel_info,
