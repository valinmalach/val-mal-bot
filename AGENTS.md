...
because neither reviewer does one itself.** Sourcery re-reviews each commit in part and stops
after five, so the workflow posts `@sourcery-ai review` with `SOURCERY_TRIGGER_TOKEN`, a
fine-grained personal access token with no expiry (Sourcery ignores comments from other
bots), skipping `opened` and drafts because Sourcery reviews a PR when it opens. Codacy's AI
Reviewer posts only when triggered, and has no every-push mode, so its `ai-reviewer/trigger`
endpoint is called with `CODACY_API_TOKEN`, an account token (`CODACY_PROJECT_TOKEN`, which
uploads coverage, does not authorise that call), on `opened`, `synchronize`,
`ready_for_review` and `reopened`. A run with no secret (a fork, Dependabot) skips the step; a
token that exists but is rejected fails it, which is how a revoked one gets noticed. The
workflow's own comments say why the Codacy call is not retried.
...
**A subject per file, and no file holding six.** A directory is a concern, not a place
to put whatever is left, and each value sits with the code that uses it — which is why
there is no `helper.py`, and the one `constants.py` (`valmal/twitch/eventsub/`) holds only
the signature header names. **A name is imported from the module that owns
it:** no package `__init__` re-exports what is beneath it, so a reader lands on the file
that defines it. The package `__init__` files are docstrings that say what the directory
is for. `valmal/db/models/__init__.py` is the exception, because importing it is what
registers the tables on the metadata Alembic reads.
...
silently. `on_ready` fires again whenever a gateway session cannot be resumed, so it
answers two questions with two flags: `_started` (has the Helix/DB-heavy work in
`run_background_tasks` *succeeded*; cleared on failure so a reconnect retries rather than
stranding a live alert or the stream session) and `_announced` (has the startup notice
been *delivered*). Anything wanted once per process belongs in `setup_hook`.
...
event. The floor is dispatch-only: a `discord.ui.View`/button callback that raises
reaches `View.on_error`, which just logs.
...
call site, so a database edit takes effect on the next restart, not before.
`valmal/db/README.md` records which side each value falls on and why; keep new values on
...
it names no HTTP type at all, and only `main.py` and the admin cog import a router.
`valmal/twitch/oauth/router.py` carries the two authorization-code flows on their own
router; it shares nothing with the webhook router, not the signature check, the models,
or the path prefix.
...
`event_sub.subscription`. `process_webhook` is generic in the model, which binds it to
its handler: pairing `StreamOnlineEventSub` with the follow handler stops type-checking.
...
403. A message id that has already reached a handler answers 202 without dispatching
again, since Twitch says a notification may arrive twice and 202 is what stops the
retries. Only ids that were *dispatched* are remembered, so a delivery Twitch retries
because this end failed still gets through. The set is process-local, remembered for
twice the freshness window, and holds 20,000 ids — the chat route claims one per chat
line — with reaching the cap said in the admin channel. The comments in `replay.py`
carry the reasoning. A 403 is a failed delivery, and enough of them revoke the
subscription, so the check sits *below* the verification handshake and applies to
notifications alone: a wrong clock must not also refuse the resubscribe that repairs it.
...
Twitch documents no 4xx/5xx distinction, and revocation counts anything that is
not a 2xx, so a 400 spends the failure budget exactly as a 500 does. That wrong
justification was recorded here first; do not restore it.
...
**The migration is destructive, and is built around that.** The reasoning is in the
...
- Twitch's uniqueness key is the type and the condition, not the transport, so repointing
  is delete-then-create. The complete definition of every subscription goes to the admin
  channel as a file *before* anything is deleted; a failure is reported per subscription
  and does not abandon the rest; a dry run is what you get unless you pass `confirm`; a
  type with no route is reported and **left alone**.
- `migrate_plan.py` holds `decide`, `Outcome` and everything that renders one, and touches
  neither Helix nor Discord; `migrate.py` does the I/O and imports the plan, never the
  other way. `decide` is pure and reads the callback but **deliberately ignores the
  status**: a correct callback that is not delivering is already `undeliverable`, which
  `recheck_subscriptions` owns. Do not restore the status condition.
- A 409 on the recreate is checked against Helix (`_exists_at`) before it is called a loss,
...
- `SubscriptionCondition` is the one model that keeps what it does not declare
  (`extra="allow"`), because a condition is *round-tripped* to recreate a subscription and
  a dropped key would recreate one broader than the original — the opposite of the rule
  for payload models above, which are read, not written back.
- `condition_of` drops every value that is `""`, because Twitch sends `""` for the unset
  half of a `channel.raid` condition and refuses a create carrying both halves; a faithful
  round-trip deletes both raid subscriptions and recreates neither. Compared against `""`
  and not tested for falsity, so a `0` or `False` survives. **Do not make this round-trip
  faithful again.**
- `WEBHOOK_PATHS` is derived from the `_route` table, not written beside it, and is passed
  *into* `migrate` rather than imported, because nothing under `valmal/twitch/` imports a
  router; `valmal/bot/cogs/twitch_admin.py` hands it over.

### Stream

**A live alert is closed by its updater, never by a webhook.** `stream.offline` carries no
stream id, so the handler cannot tell which stream ended; it calls `live_alert.wake()`, and
the updater re-checks Helix and stops if it has been superseded. The same webhook calls
`stream_session.wake()`, which asks Helix the same question for a different scope. Both guard
against being superseded. `docs/adr/0001-alert-updater-is-the-only-closer.md` and
`docs/adr/0004-the-stream-session-ends-itself.md` have the why. The rule the cycle turns on
is `live_alert_cycle._decide`, which is pure — put new conditions there, not in the
surrounding I/O. `live_alert.py` is the lifecycle (start an updater, wake it, restore them
after a restart); `live_alert_cycle.py` is one pass and knows nothing about tasks.

**`live_alert` and `stream_session` are different scopes.** A live alert exists per
broadcaster and is kept in Postgres, because it names a Discord message that outlives a
redeploy; a stream session is the main broadcaster being live, holds the `Stream` it began
for, and owns everything lasting exactly one stream — the queue's contents, the ad-break
warning and the settled ids — in memory. Only a stream Helix confirms is gone stands a
session down, and the alert updater no longer asks. `stream_session` owns every transition
of its own: `began` and `resume` take a session up, `wake` is the only thing that can bring
one down, and both ends are private to the module. `resume` is not a second `began`:
nothing is greeted, because the stream did not just start. Beginning resets whatever a
previous session left, which is also how a session nothing could end recovers at the next
`stream.online`.

**An autoshoutout is one per person per stream, and the session remembers who.**
`valmal/twitch/stream/autoshoutout.py` owns the list, the rule and the four places someone
can turn up. The rule is `_decide` and is pure; the lookup is *inside* it, as `LOOK_UP`,
because the cost guarantee — one query per distinct chatter per stream — is the rule rather
than an optimisation around it. The session holds one set of **settled** ids, not two:
*spent* and "looked up and not on the list" both mean "do not ask again this stream".

It shouts nobody out itself. It posts `!so <login>`, the round trip `channel_raid` already
uses, so there is one implementation of what a shoutout is — at the price of the wording
being `!so`'s. An incoming raid is settled rather than shouted, because the raid handler's
own `!so` already gave them one; that mark lives in `channel_raid`, not in the shoutout
handler, which a mod's manual `!so` also reaches. `!aso` is the exception that calls
`shoutout` directly. A redemption takes the ordinary path: any custom reward counts,
including one still queued or later refunded, because the point is that they turned up.
The shared-chat guard runs before anything reads a chat line, so a line relayed from
another channel is dropped first.

**One drainer, for the life of the process.** A pass that fails costs that pass, not the
stream's shoutouts. It asks the session each pass whether it may send: a target whose lookup
failed re-queues itself, and would otherwise be shouted out into an offline chat.
`shoutout_queue` therefore imports `stream_session` inside `drain`, because the session
imports the queue to empty it.
...
set, `next_birthday` from the instant when it is being rolled forward, and the second asks
the first (it reads the local date back off the instant using `birthday_timezone`). Only a
row written before that column existed has no zone, and falls back to bumping the year. A
timezone name the tz database has dropped is cleared with a notice rather than raised:
`next_birthday` runs one line *before* the write that reschedules the record, so a raise
there is a birthday that never moves and is never greeted. `is_leap_day` is derived
...
`upsert_user`. `docs/adr/0003-birthday-holds-the-next-occurrence.md` records why the
...
- **An audit entry's sentences are `message_template` rows; what fills them is
  ...
  footers, a description that is only values, and the values a template is filled
  with (names, mentions, times, and the marker for an absent value, `UNKNOWN_USER`
  or `Never`). A sentence that flips on a condition — ban or unban, pinned or
  unpinned, one role or several — is one row per variant, not one row with a
  fragment passed in, because a fragment hides English in code. A new entry adds
  its rows in a migration.
- **No audit entry can build a value Discord will reject.** Capping happens in
  ...
  by forgetting. It is enforced rather than documented because the documented
  version was false: a member's roles at 43 mentions cost the entry *and* the row
  deletion queued after it. Footers are bounded by construction, being ids and
  fixed text.
- **What a person wrote is escaped; what the bot wrote is not.** `_said` for a
  ...
  content that may instead be the bot's not-found marker. A description and a field
  value both render masked links, so an unescaped message could put a label of its
  choosing on a URL of its choosing in front of whoever reads the audit log. An
  **author line** and a **footer** are plain text to Discord and are deliberately left
  alone, which is why a name is escaped in one place on an entry and not the other.
- **Everything the bot says about itself goes through `valmal/core/errors.py`.** `report(exc,
  ...
  to do, and two inside one window are two different records.
...
- **Twitch chat never carries those two placeholders, on purpose.** It cannot show
  ...
  written literally, and `say_template` goes through `config.template`, which
  renders, so it would go out as a raw `<#id>`. Neither is guarded; nothing seeded
  does it.
...
- **Comments record a non-obvious *why*, or do not exist.** Match the density in
  `valmal/db/base.py` and `valmal/db/config.py`; do not narrate what the code already
  says, and do not retell what the code used to do — a commit or an ADR holds that. A
  rule that must not be undone (do not restore the status condition) keeps one line.
